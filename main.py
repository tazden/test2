...
def process():
        ...
        if 10<len(requesttg)<=20:
            ...
            payment.append(start)
            timing.append(request.json["message"]["date"])
        with open("infa.csv", mode="w", encoding='utf-8') as w_file:
            file_writer = csv.writer(w_file, delimiter=",", lineterminator="\r")
            file_writer.writerow(["Имя", "Номер", "айди","дата и время оплаты(если есть)","какую сумму заплатил"])
            for i in range(len(requesttg)):
                file_writer.writerow([str(imya[i]), str(numbers[i]), str(requesttg[i]), str(timing[i]), str(payment[i])])
        return {"ok": False}
